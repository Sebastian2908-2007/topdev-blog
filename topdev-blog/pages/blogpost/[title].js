...
export default function Post ({post}) {
 ...
 // console.log(postObj,"Pst OBJ");
    let html = postObj.html.replace(
      '<body ',
      '<section class="d-flex flex-column justify-content-between align-items-center m-4 postSec text-light text-center p-1 " ');
    html = html.replace('</body>','</section>');  
    html = html.replace('<img','<img class="img-fluid"');  
    /* data image thumnail path for future use maybe Note social sites
    do not accept base64 for og tags ill just keep this here for now for reference
    const iconBase = postObj.thumbNail;
    const iconBase2 = iconBase.split('<img src=')
    const iconBase3 = iconBase2[1].split('/>')[0];
    console.log(iconBase3);
    const iconBase4 = iconBase3.replace('"','');
    const iconBase5 = iconBase4.replace('"','');*/
    ...
    <Head>
     ...
     <meta name="keywords" content={postObj.keywords}/>
    </Head>
    <div className="m-4 d-flex flex-row justify-content-between">
     ...
     <button onClick={() => {setOpenShareModal(true); setModalInfo({image:'/topdev-logo.png',title:`${postObj.title}`});}} className="p-2 rounded-circle like-share-icon">
      ...
      {parse(html)}
      <section className="d-flex comments flex-column justify-content-between align-items-center m-4 bg-light text-center p-1" >
